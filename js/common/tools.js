var uid=window.jsObj.getUserid();
...
function modal() {
	...
	modal = '<div class="modalbox">' +
		'<span class="cha"></span><div class="shuru">' +
		'<p class="error">温馨提示：账号<span class="red">提现</span>需绑定手机号确认</p><div>' +
		'<input type="text" id="phone" class="inputs" placeholder="请输入你的手机"/></div><div>' +
		'<input type="text" id="code" class="inputs" placeholder="请输入验证码"/><button class="yzm" id="yzm">获取验证码 </button></div>' +
		'<div class="submit">确认绑定</div></div></div>';
	$("#modal").html(modal);
}
var wait = 120;
//封装限制用户恶意发送短信方法
