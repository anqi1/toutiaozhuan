...
var uid="55311b8cf24548be8ef0d13faa3355de";
...
var urllocation="http://192.168.0.102:8003";
...
function ifwithdraw(){
	requestMenthodGet("selectHasBind/"+uid).done(function(data){
		//没有电话号码并且没有得到openid
		if(data.data.phone==1&&data.data.openid==1){
		...
		}else if(data.data.phone==0&&data.data.openid==1){
			if($("title").text()=="我的余额"){
			...
			window.location="canel/weixin.html";
		}
	})
}
	//封装限制用户恶意发送短信方法
